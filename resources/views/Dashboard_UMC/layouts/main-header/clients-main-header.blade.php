						...
						<div class="nav nav-item  navbar-nav-right ml-auto">
                                    ...
                                    <div class="menu-header-content bg-primary text-right">
                                        <div class="d-flex">
                                            ...
                                            <span class="badge badge-pill badge-warning mr-auto my-auto float-left"
                                                id="markAllRead" style="cursor:pointer">Mark All Read</span>
                                        </div>
                                        <p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">
                                            You have (<span id="notificationsCount">{{ Auth::guard('clients')->user()->unreadNotifications->count() }}</span>) unread Notifications
                                        </p>
                                    </div>

                                    <div class="main-notification-list Notification-scroll" id="notificationsList">
                                        @forelse(Auth::guard('clients')->user()->unreadNotifications as $notification)
                                            @php
                                                ...
                                                $statusIcons = [
                                                    'accepted' => 'la la-check-circle',
                                                    'partially_accepted' => 'la la-exclamation-circle',
                                                    'rejected' => 'la la-times-circle'
                                                ];
                                            @endphp
                                            {{-- <a class="d-flex p-3 border-bottom" href="{{ route('order.show', $data['order_id'] ?? '#') }}"> --}}
                                            <a class="d-flex p-3 border-bottom" href="#">
                                                <div class="notifyimg {{ $statusColors[$data['final_status']] ?? 'bg-primary' }}">
                                                    <i class="{{ $statusIcons[$data['final_status']] ?? 'la la-info-circle' }} text-white"></i>
                                                </div>
                                                <div class="mr-3">
                                                    <h5 class="notification-label mb-1">{{ $data['message'] ?? 'No message' }}</h5>
							...
							<div class="dropdown main-profile-menu nav nav-item nav-link">
                                    <?php
                                        ...
                                        $imageclient = Client::query()->where('id', '=', Auth::guard('clients')->user()->id)->first();
                                    ?>
                                        @if (empty($imageclient->image))
                                            <a class="profile-user d-flex" href=""><img alt="" src="{{URL::asset('assets/img/faces/6.jpg')}}"></a>
                                        @else
                                            <a class="profile-user d-flex" href=""><img src="{{URL::asset('storage/'.$imageclient->image)}}"></a>
                                        @endif
                                        <div class="dropdown-menu">
                                            <div class="main-header-profile bg-primary p-3">
                                                <div class="d-flex wd-100p">
                                                    ...
                                                    @else
                                                        <div class="main-img-user"><img alt="" src="{{URL::asset('storage/'.$imageclient->image)}}" class=""></div>
                                                    @endif
                                            ...
                                            <a class="dropdown-item" href="{{ route('profileclient.edit') }}"><i class="bx bx-user-circle"></i>{{__('Dashboard/profile.Update Profile')}}</a>
                                            <a class="dropdown-item" href=""><i class="bx bxs-inbox"></i>Inbox</a>
                                            <a class="dropdown-item" href=""><i class="bx bx-envelope"></i>Messages</a>
                                            <a class="dropdown-item" href=""><i class="bx bx-slider-alt"></i> Account Settings</a>
                                            <form method="POST" action="{{ route('logout.clients') }}">
                                                @csrf
